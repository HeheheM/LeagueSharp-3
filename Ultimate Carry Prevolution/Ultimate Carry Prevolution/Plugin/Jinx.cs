    ...
    class Jinx : Champion
    {
        ...
        private void LoadMenu()
        {
			var champMenu = new Menu("Jinx Plugin", "Jinx");
            {
				...
				//	{
				//		rMenu.AddItem(new MenuItem("R_Min_Range", "R Min Range Slider").SetValue(new Slider(300, 1, 950)));
				//		rMenu.AddItem(new MenuItem("R_Max_Range", "R Max Range Slider").SetValue(new Slider(900, 300, 950)));
		...
		//			var E_Behind_Vec_Slow = E_Pred_Slow.CastPosition + Vector3.Normalize(unit.ServerPosition - MyHero.ServerPosition) * 100;
		...
		//		var E_Pred = E.GetPrediction(unit);
		//		var E_Behind_Vec = E_Pred.CastPosition + Vector3.Normalize(unit.ServerPosition - MyHero.ServerPosition) * E_Behind_Distance;
